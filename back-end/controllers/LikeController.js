...
class LikeController {
  ...
  static async CreateLike(req, res) {
    ...
    const existingLike = Like.findOne({ userId, parentId });
  ...
  static async DeleteLike(req, res) {
    const { user } = req;
    const parentId = req.params.recipeId;

    const rec = await Recipe.findOne({ _id: parentId, userId: user.id });
